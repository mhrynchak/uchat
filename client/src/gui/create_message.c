...
static gboolean auth_get_data(cJSON *msg, cJSON **data, char *field, 
...
static t_groom *help_create_message(gboolean *valid, cJSON *msg, t_gmsg *gmsg,
    ...
    if (*valid && (*valid = auth_get_data(msg, &data, "message", cJSON_IsString)))
        gmsg->msg = strdup(data->valuestring);
    if (*valid && (*valid = auth_get_data(msg, &data, "room_id", cJSON_IsNumber)))
    ...
    if (!groom)
        return NULL;
    if (*valid && (*valid = auth_get_data(msg, &data, "user_id", cJSON_IsNumber))) {
        gpointer hash = g_hash_table_lookup(groom->members,
                                            GINT_TO_POINTER(data->valueint));
        gmsg->login = g_strdup(MX_J_STR(hash));
    }
    if (*valid && (*valid = auth_get_data(msg, &data, "date", cJSON_IsNumber)))
        gmsg->date = data->valuedouble;
    return groom;
}

t_gmsg *create_gmsg(cJSON *msg, t_chat *chat) {
    t_gmsg *gmsg = malloc(sizeof(t_gmsg));
    cJSON *data = NULL;
    gboolean valid = TRUE;
    t_groom *groom = NULL;

    groom = help_create_message(&valid, msg, gmsg, chat);
    if (valid && (valid = auth_get_data(msg, &data, "message_id", cJSON_IsNumber)))
        gmsg->message_id = data->valuedouble;
    if (valid && (valid = auth_get_data(msg, &data, "msg_type", cJSON_IsNumber)))
